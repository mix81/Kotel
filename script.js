...
// Функция подключения с защитой от "засыпания"
function connect() { 
    mqtt.connect({ 
        userName: cfg.u, 
        password: cfg.w, 
        ...
        keepAliveInterval: 30,      // Пинг сервера каждые 30 секунд
        onSuccess: () => { 
...
// Обработчик потери связи (для мгновенной реакции)
mqtt.onConnectionLost = (responseObject) => {
    if (responseObject.errorCode !== 0) {
        console.log("Связь потеряна, восстановление...");
        setTimeout(connect, 2000);
    }
};

// Проверка связи при активации экрана телефона (Visibility API)
document.addEventListener('visibilitychange', () => {
    if (document.visibilityState === 'visible') {
        if (!mqtt.isConnected()) {
            console.log("Приложение активно, переподключаем MQTT...");
            connect();
        }
    }
});
...
// Статус окон и пульсация температуры
...
// Обновление состояния кнопок и свечения карточек
